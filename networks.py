from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, UpSampling2D, Flatten, Reshape, BatchNormalization, \
    Dropout
from tensorflow.keras import regularizers


def Net_Encoder(input_img, weight_decay=0.0001, add_dense=True):
    net = Conv2D(32, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_1')(input_img)
    ...
    net = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_2')(net)
    net = BatchNormalization()(net)
    net = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_2_2')(net)
    ...
    net = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_3')(net)
    net = BatchNormalization()(net)
    net = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_3_2')(net)
    ...
    net = Conv2D(256, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_4')(net)
    net = BatchNormalization()(net)
    net = Conv2D(256, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='conv_4_2')(net)
...
def Net_Decoder(encoder, weight_decay=0.0001, dense_added=True):
    ...
    net = Conv2D(32, (3, 3), activation='relu', padding='same', kernel_regularizer=regularizers.l2(weight_decay),
                 name='upconv_1_2')(net)
    net = BatchNormalization()(net)
    net = UpSampling2D((2, 2), name='upsample_0')(net)
    net = Dropout(0.1)(net)
